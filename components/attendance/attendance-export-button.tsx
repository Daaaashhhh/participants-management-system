...
import { AttendanceExportSettingsModal } from './attendance-export-settings-modal';
...
export function AttendanceExportButton({ records, eventDate }: AttendanceExportButtonProps) {
  ...
  const handleConfirmExcelExport = async (settings: AttendanceExportSettings) => {
    setIsSettingsModalOpen(false);
    try {
      await exportAttendanceToExcel(records, eventDate, settings);
      const pages = Math.max(1, Math.ceil(records.length / 15));
      success(
        `Exported ${records.length} attendee records across ${pages} page${
          pages === 1 ? '' : 's'
        } to Excel (.xlsx).`
      );
    } catch (err) {
      console.error('Export error:', err);
      error('Failed to generate DSWD Excel attendance sheet.');
    }
  };
    ...
    <>
      <div className="relative inline-block text-left" ref={dropdownRef}>
        ...
        >
          ...
          <ChevronDown
            className={`h-3.5 w-3.5 ml-1 text-slate-400 transition-transform ${
              isOpen ? 'rotate-180' : ''
            }`}
          />
        </Button>
          ...
          <div className="absolute right-0 mt-2 w-72 rounded-xl bg-white shadow-xl border border-slate-200 z-50 py-2 focus:outline-none animate-in fade-in-50 zoom-in-95 duration-100">
            <div className="px-3 py-2 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                Official DSWD attendance sheet template
              </p>
            </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 group-hover:text-emerald-700 mt-0.5">
                    Official DSWD template (15/page)
                  </p>
                </div>
              </button>
